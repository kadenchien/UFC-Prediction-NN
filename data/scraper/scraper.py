...
def scrape_fight_page(url):
    ...
    fight_data = {}
    if soup.find('h2', class_='b-content__title') is not None:
        fight_data['event'] = soup.find('h2', class_='b-content__title').text.strip()
        print("Scraping " + fight_data['event'] +"...")
    if soup.find_all('a', class_='b-link b-fight-details__person-link') is not None:
    ...
    if soup.find_all('i', class_='b-fight-details__person-status') is not None:
    ...
    if stats_tables is not None and len(stats_tables) >= 2:
        # Totals table
...
def scrape_ufc_events(url, num_events):
    ...
    for page in range(num_events):
        ...
        for i in range(len(fightnights)):
            if "Fight Night" not in fightnights[i].text.strip() and "Ultimate Fighter" not in fightnights[i].text.strip():
...
ufc_data = scrape_ufc_events(url, num_events=13)  # Adjust num_events for more data

# Converting to DataFrame and saving to CSV
df = pd.DataFrame(ufc_data)
df.to_csv('data/raw/ufc_fight_data_2.csv', index=False)
